...
public class MetricsConfiguration extends MetricsConfigurerAdapter implements ServletContextInitializer {
    ...
    public MetricsConfiguration(MicroMonitorProperties microMonitorProperties) {
    ...
    public void init() {
        ...
        if (microMonitorProperties.getMetrics().getJmx().isEnabled()) {
            log.debug("Initializing Metrics JMX reporting");
            JmxReporter jmxReporter = JmxReporter.forRegistry(metricRegistry).build();
            jmxReporter.start();
        }
        if (microMonitorProperties.getMetrics().getLogs().isEnabled()) {
            ...
            final Slf4jReporter reporter = Slf4jReporter.forRegistry(metricRegistry)
                ...
                .build();
            reporter.start(microMonitorProperties.getMetrics().getLogs().getReportFrequency(), TimeUnit.SECONDS);
        }
    }
    ...
    public void onStartup(ServletContext servletContext) {
        ...
        if (microMonitorProperties.getMetrics().getPrometheus().isEnabled()) {
            String endpoint = microMonitorProperties.getMetrics().getPrometheus().getEndpoint();
